...
@ext.get("/add")
async def add(
    url: str, request: fastapi.Request
    ...
    INSERT INTO redirect(
        ...
        return fastapi.responses.PlainTextResponse(
            "Error: Failed to save to database", 500
        )
...
DELETE FROM redirect WHERE id = ?
        ...
        await db.rollback()
        return (
            fastapi.responses.PlainTextResponse("No redirect url with that ID found!"),
            500,
        )
...
async def modify(id: str, url: str) -> fastapi.responses.PlainTextResponse:
    ...
    """
    try:
        await db.execute("UPDATE redirect SET url = ? WHERE id = ?", (url, id))
        await db.commit()
    except sqlite3.OperationalError:
        await db.rollback()
...
async def listing() -> fastapi.responses.PlainTextResponse:
    ...
    if len(con) == 0:
        return fastapi.responses.PlainTextResponse(
            "No redirect URLs has been added yet.", 404
        )
